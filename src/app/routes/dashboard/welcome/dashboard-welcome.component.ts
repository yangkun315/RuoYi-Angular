...
@Component({
  ...
  standalone: true,
  imports: [NzGridModule, NzCardModule, NgxEchartsDirective],
...
export class DashboardWelcomeComponent implements OnInit {
  ...
  readonly statCards: Array<{ title: string; value: string; tone: KpiTone; spark: number[] }> = [
    {
      title: '网站流量',
      value: '123,456',
      tone: 'blue',
      spark: [42, 68, 55, 72, 61, 78, 52, 85, 48, 70, 63, 75],
    },
    {
      title: '展示量',
      value: '234,567K',
      tone: 'green',
      spark: [55, 48, 62, 58, 70, 65, 52, 80, 72, 60, 68, 77],
    },
    {
      title: '销售总额',
      value: '¥458,778',
      tone: 'orange',
      spark: [38, 52, 45, 58, 50, 68, 62, 55, 72, 48, 60, 66],
    },
    {
      title: '待处理工单',
      value: '456',
      tone: 'magenta',
      spark: [48, 55, 42, 60, 52, 58, 45, 68, 50, 62, 56, 48],
    },
  ];
    ...
    series: [
      {
        type: 'bar',
        barMaxWidth: 28,
        data: [320, 450, 380, 520, 610, 480, 590, 720, 680, 540, 620, 700],
      },
    ],
  };
  ...
  ngOnInit(): void {
    void this.loadMapsWhenNeeded();
  }

  /** 地图相关 ECharts 模块单独 chunk，仅进入默认页时加载 */
  private async loadMapsWhenNeeded(): Promise<void> {
    await import('../../../core/echarts-maps');
